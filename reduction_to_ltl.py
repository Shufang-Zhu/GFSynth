...
if __name__ == '__main__':
	parameters = sys.argv
	if len(parameters) != 3:
		print("Usage: <executable-strix-location> <benchmark-name>")

	strix_location = parameters[1]
	benchmark_lication = parameters[2]
	env_gr1_file = benchmark_lication + "_env_fst.env_gr1";
	env_safety_file = benchmark_lication + "_env_fst.env_safety";
	agn_reach_file = benchmark_lication + "_env_fst.agn_reach";
	agn_safety_file = benchmark_lication + "_env_fst.agn_safety"
	part_file = benchmark_lication + "_env_fst.p";
	...
	in_vars, out_vars = read_part_file(part_file)


	ltl = ltlfgr1_to_ltl(env_gr1_ltl, env_safety_ltlf, agn_reach_ltlf, agn_safety_ltlf, benchmark_lication)


	run_strix = strix_location + " -k -t --ins=\"" + in_vars + "\" --outs=\"" + out_vars + "\" -f \"" + ltl + "\""
	# print(run_strix)
	os.system(run_strix)
