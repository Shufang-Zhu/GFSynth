...
namespace Syft {
    ...
    LTLfGR1Synthesizer::LTLfGR1Synthesizer(std::shared_ptr<VarMgr> var_mgr, GR1 gr1, SymbolicStateDfa env_safety,
    ...
    void LTLfGR1Synthesizer::print_variables(const SymbolicStateDfa arena, const std::string& filename) const{
        ...
        if(to_slugs.is_open()) {
            ...
            for (std::string output_variable_label : output_variable_labels) {
                to_slugs << boost::algorithm::to_lower_copy(output_variable_label) << "\n";
            }
            to_slugs << "\n";

            to_slugs << "[OUTPUT]\n";
            for (std::string input_variable_label : input_variable_labels) {
                to_slugs << boost::algorithm::to_lower_copy(input_variable_label) << "\n";
            }
    ...
    void LTLfGR1Synthesizer::print_initial_conditions(const CUDD::BDD& arena_initial_state_bdd, const std::string& filename) const{
        std::ofstream to_slugs;
        ...
        if(to_slugs.is_open()) {
            ...
            to_slugs<<boost::algorithm::to_lower_copy(var_mgr_->bdd_to_string(arena_initial_state_bdd));
            to_slugs<<"\n\n";
    ...
    void LTLfGR1Synthesizer::print_transitions(const SymbolicStateDfa arena, const std::string& filename) const {
        std::ofstream to_slugs;
        to_slugs.open(filename, std::ios::app);
        if(to_slugs.is_open()) {
            ...
            for (size_t i = 0; i < transition_function.size(); i++){
                ...
                to_slugs<<boost::algorithm::to_lower_copy(tran)<<"\n";
            }

            std::string safe_states_string = var_mgr_->bdd_to_string(safe_states_);
            to_slugs<<boost::algorithm::to_lower_copy(safe_states_string)<<"\n";
    ...
    void LTLfGR1Synthesizer::print_liveness_constraints(const SymbolicStateDfa arena, const std::string& filename) const {
        std::ofstream to_slugs;
        to_slugs.open(filename, std::ios::app);
        if(to_slugs.is_open()) {
            ...
            for (CUDD::BDD justice : gr1_.env_justices){
                if (justice == var_mgr_->cudd_mgr()->bddOne() || justice == var_mgr_->cudd_mgr()->bddZero()) {
                    to_slugs<<boost::algorithm::to_upper_copy(var_mgr_->bdd_to_string(justice))<<"\n";
                }
                else{
                    to_slugs<<boost::algorithm::to_lower_copy(var_mgr_->bdd_to_string(justice))<<"\n";
                }
            }
            to_slugs<<"\n\n";
    ...
    const std::string LTLfGR1Synthesizer::exec_slugs(const std::string& slugs, const std::string& slugs_input_file, const std::string& slugs_res_file, const std::string& slugs_strategy_file) {
        std::string result;
        ...
        boost::split(substr, substr[1], boost::is_any_of(" "));
        if (substr[2] == "unrealizable."){
            result = "unrealizable";
    ...
    SynthesisResult LTLfGR1Synthesizer::run(std::string& benchmark_name) {
